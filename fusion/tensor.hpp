...
struct DevBuffer {
  ...
  DevBuffer(size_t size) : size(size) { data = device_alloc(size); }
...
struct TensorDesc : public Dim {
  ...
  // updates the `Dim` fields by reading the descriptor `desc` with
  // Get4dTensorDescriptor
  void update_get() {
    miopenDataType_t dt;
    int ns, cs, hs, ws;
    CHECK_MIO(miopenGet4dTensorDescriptor(desc, &dt, &n, &c, &h, &w, &ns, &cs,
                                          &hs, &ws));
...
struct Tensor : public TensorDesc {
  ...
  Tensor(Tensor &&o)
      : TensorDesc(std::move(o)), data(o.data), data_size(o.data_size),
  ...
  Tensor(TensorDesc &&d)
      : TensorDesc(std::move(d)),
        data_size(n * (size_t)c * h * w * sizeof(float)), owns_data(true) {
    alloc();
  }

  Tensor(const Dim &dims)
      : TensorDesc(dims), data_size(n * (size_t)c * h * w * sizeof(float)),
        owns_data(true) {
    alloc();
  }
  ...
  Tensor(int n, int c, int h, int w, bool do_alloc)
      : TensorDesc(n, c, h, w),
        data_size(n * (size_t)c * h * w * sizeof(float)), owns_data(do_alloc) {
    if (do_alloc) {
      alloc();
    }
  }

  // reshape (creates a tensor object of new dimensions that doesn't own its
  // data)
